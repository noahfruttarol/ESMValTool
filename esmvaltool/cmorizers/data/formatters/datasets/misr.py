...
def bunch_files_by_year(in_dir, files_pattern):
    """Group files by year."""
    files_by_year = {}
    for filepath in Path(os.path.join(in_dir)).glob(files_pattern):
...
def _extract_variable(short_name, var, cfg, in_dir, out_dir):
    ...
    files_by_year = bunch_files_by_year(in_dir, files)
    for year, filepaths in files_by_year.items():
        cubes = iris.cube.CubeList()
        for filepath in filepaths:
            xrds = xarray.open_dataset(
                filepath, group="Aerosol_Parameter_Average"
            )
            ...
            days_since_1850 = datetime.date(year, month, 15) - datetime.date(
                1850, 1, 1
            )
            lb_since_1850 = datetime.date(year, month, 1) - datetime.date(
                1850, 1, 1
            )
            if month == 12:
                ub_since_1850 = (
            ...
            cube.coord("time").units = "days since 1850-01-01 00:00:00"
